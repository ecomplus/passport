...
function readResource (storeId, callback, endpoint) {
  let method = 'GET'
  ...
  callApi(endpoint, method, null, storeId, callback, successCallback)
}

// fields returned from customer resource object
// https://ecomstore.docs.apiary.io/#reference/customers/customer-object
const customerFields = '_id,locale,main_email,accepts_marketing,display_name,birth_date,gender,photos'
...
function findCustomer (storeId, provider, userId, verifiedEmail, callback) {
  ...
  let endpoint = 'customers.json' +
    ...
    '&fields=' + customerFields + ',oauth_providers'
  let method = 'GET'
...
function deleteCustomer (storeId, customerId, callback) {
  let method = 'DELETE'
  modifyCustomer(storeId, customerId, null, callback, method)
}
...
function crud (storeId, customerId, method, resource, resourceId, body, callback) {
  // abstract requests to other resources
  // create, read, update, delete
  let checkCustomer = (err, body) => {
    if (!err) {
      if (body) {
  ...
  let endpoint = resource + '/' + resourceId + '.json'
  readResource(storeId, checkCustomer, endpoint)
}
