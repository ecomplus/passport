...
function requestEnd (endpoint, res, rawData, callback, successCallback, passError) {
...
function callApi(endpoint, method, body, storeId, callback, successCallback, passError) {
  ...
  if (body) {
    ...
    if (body) {
      ...
      // do not permit to edit some order properties directly
      delete body.financial_status
      delete body.fulfillment_status
      delete body.shipping_lines
      delete body.transactions
      delete body.payments_history
      delete body.fulfillments
      delete body.loyalty_points
      delete body.loyalty_points_entries
    ...
    if (callback !== undefined) {
      ...
      res.on('end', () => {
        if (res.statusCode === 204) {
          ...
          requestEnd(endpoint, res, rawData, callback, SuccessCallback, passError)
        }
      })
    } else {
...
const publicCustomerFields = '_id' +
  ',enabled' +
  ',login' +
  ',oauth_providers' +
  ',locale' +
...
function findCustomerByEmail (storeId, email, docNumber, callback) {
  ...
  endpoint += `&fields=${publicCustomerFields}`
  let method = 'GET'
  callApi(endpoint, method, null, storeId, callback, (data) => {
    // 200 OK
    if (typeof data === 'object' && data !== null && Array.isArray(data.result)) {
      ...
      if (customers.length) {
        ...
        if (customers.length === 1) {
          ...
          customer = customers.find(({ orders }) => orders && orders.length) ||
...
function createCustomer (storeId, profile, callback) {
  ...
  const sendPost = () => {
    ...
    }, Boolean(profile.main_email))
  }

  if (profile.main_email) {
    // already parsed
    body.main_email = profile.main_email
    sendPost()
  } else if (!Array.isArray(profile.emails) || profile.emails.length === 0) {
    // random
...
function crud(storeId, customerId, method, resource, resourceId, endpoint, querystring, body, callback) {
  ...
  if (resourceId) {
    ...
    if (method === 'GET') {
      ...
      endpoint = resource + '.json?sort=-updated_at&'
      if (resource === 'orders') {
        // https://ecomstore.docs.apiary.io/#reference/orders
        endpoint += 'buyers._id='
        if (querystring) {
          querystring = querystring.replace(/&?buyers\._id=[^&]+/g, '')
        }
      } else {
        // carts ?
        ...
        endpoint += 'customers='
        if (querystring) {
          querystring = querystring.replace(/&?customers=[^&]+/g, '')
        }
      }
      endpoint += customerId
      if (querystring) {
