  ...
  callApi(endpoint, method, null, storeId, callback, (data) => {
    ...
    if (verifiedEmail) {
      ...
      findCustomerByEmail(storeId, verifiedEmail, (err, id, customer) => {
        if (!err && id) {
          // customer found
          // check if this oauth provider is already in customer document
          let accounts = customer.oauth_providers
          let checked
          if (Array.isArray(accounts)) {
            for (let i = 0; i < accounts.length; i++) {
              if (accounts.user_id === userId && accounts.provider === provider) {
                checked = true
              }
            }
          }
