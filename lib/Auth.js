...
module.exports = {
  ...
  generateToken (storeId, customerId, authLevel = 1, canCreateOrder = true) {
  ...
  validate (customerId, storeId, token) {
    ...
    let { authLevel, canCreateOrder, expires } = decoded
    if (!authLevel) {
      authLevel = decoded.level
    }
    if (typeof authLevel === 'number' && expires > Date.now()) {
      if (decoded.customer_id === customerId) {
        if (decoded.store_id === storeId) {
          // valid access token and ID
          return { authLevel, canCreateOrder }
        } else {
          return {
            error: 4,
